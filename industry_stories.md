...
Fourteen years in enterprise software — chasing down bad data in client warehouses, shipping product roadmaps, and learning what the textbooks leave out.

---

## Intro

I came to academic research the long way around. Before I ever set foot in a PhD program, I spent over a decade inside the operations of global companies — first as a consultant and solution manager deploying supply chain software at i2 Technologies, then as Director of Product Management at Servigistics, building the planning tools that aftermarket service organizations depend on.

This page is not a résumé entry. It's an attempt to connect the dots — to explain why the research questions I pursue feel urgent to me in a way that goes beyond academic curiosity. The problems I study now are problems I once lived inside. For years, I wrote a professional blog in Traditional Chinese (and occasionally English) called [售後服務規劃 · Post Sales Service Planning](https://serviceplanning.wordpress.com/) documenting the technical challenges, the industry debates, and the moments where theory and practice collided. Much of what follows draws on that experience.
...
I joined i2 Technologies when the company was riding the first wave of enterprise supply chain software — before SaaS, before cloud, before the term "digital transformation" existed. i2 was building tools that promised something genuinely new: let large manufacturers see across their supply chains in real time, synchronize demand with supply, and make better decisions faster. The ambition was enormous. So were the implementation challenges.

> "The software worked beautifully in the demo. The real world had other ideas."

### What the Work Actually Looked Like

My job was to sell, configure, and deploy supply chain planning systems at Fortune 500 clients across North America and Asia-Pacific. In practice, this meant spending weeks embedded inside client operations — walking factory floors, sitting with demand planners at 6 a.m., tracing the lineage of a single data field through four legacy systems to figure out why the forecast numbers didn't match reality.

Every implementation was a renegotiation between what the software assumed and what the client's operations actually looked like. Lead times that weren't fixed. Demand signals contaminated by promotions, returns, and internal orders. Inventory policies set years ago and never revisited. The model in the system was always cleaner than the world it was supposed to represent.
...
One of the lessons I internalized early — and have written about at length — is the difference between tactical and strategic planning in supply chain contexts. It sounds obvious until you're in a client meeting where a VP is demanding a single system that does both. Tactical planning optimizes operations within a given capacity. Strategic planning asks what that capacity should be in the future. The planning horizon, the data granularity, the organizational accountability — all of it differs. You need them to talk to each other, but you cannot collapse them into one.

The best implementation I ever saw of this separation was at a major Taiwanese semiconductor foundry. Their strategic module was run by operations researchers and reported to Finance — independent, analytically serious, with real organizational teeth. Every other client I worked with blurred the line and paid for it eventually.
...
Those years shaped how I think about information asymmetry and incentive alignment in supply chains — topics that would eventually show up in my research. When a client's planning team doesn't fully share operational data with their software vendor, when they paper over problems to make the project look on track, the downstream consequences can take months to surface. I watched it happen more than once.

### What I Took Away

Supply chain theory says: minimize costs subject to service-level constraints, given some demand distribution and lead time. Practice says: figure out which data you can actually trust, negotiate with stakeholders who have competing incentives, and build a model that's wrong in predictable rather than unpredictable ways. The gap between those two statements is where most of the real difficulty lives — and it's a gap that academic research can help close, if it takes the operational messiness seriously.
...
After years of deploying other people's software, I wanted to sit on the other side of the table — to be responsible for what the product actually did, not just how it was implemented. Servigistics gave me that chance. The company built planning software specifically for aftermarket service organizations: the parts, logistics, and planning operations that keep equipment running after the original sale.

It's a domain that doesn't get as much attention as forward supply chains, but the operational complexity is just as high — and in many ways the stakes are higher. When a hospital's imaging equipment goes down, or an airline's ground service vehicle needs a part, the tolerance for stockouts is effectively zero. The cost of carrying too much inventory is also enormous. Service parts planning sits in that narrow window between the two.

### Service Parts Demand Forecasting

Most demand forecasting assumes items sell regularly enough to detect a pattern. Service parts violate that assumption constantly. A critical spare might have zero demand for six months, then a sudden spike when a fleet ages or a recall hits. The statistical methods that work well for consumer goods — moving averages, exponential smoothing, seasonal decomposition — are often useless here.
...
The academic literature on this problem — running from Sherbrooke's METRIC model through the competing frameworks of Muckstadt (Cornell) and Cohen (Wharton) — is rich and genuinely difficult. I was fortunate enough to see this academic tension play out in person when Servigistics hosted its annual eXchange sales conference and invited both Muckstadt and Cohen to present. These are two intellectual giants who had spent over a decade as competitors — academically and commercially — and the contrast in their styles was striking: Muckstadt with the rigor of an engineer (jet engine photos, dense mathematics), Cohen with the fluency of a business school professor. Each framework has genuine merits and genuine blind spots. Working on product that had to implement a version of these solutions in real customer environments meant navigating those trade-offs daily.
...
One of the least-studied corners of operations management — at least at the time I was working in it — was how to actually price a service contract. Not just what the market would bear, but what the cost structure actually supported. The challenge is that it sits at the intersection of insurance pricing (you're underwriting a risk) and services pricing (you're selling labor and expertise). Most companies I worked with were using either a naive cost-plus approach, a market-comparison approach that mostly amounted to guessing, or an "include it in the product price and figure it out later" strategy that effectively obscured all the real economics.

The aviation industry was furthest along — driven by the need to price things like Power by the Hour agreements for jet engines, where the customer pays per flight hour rather than buying the engine outright. The Total Ownership Cost (TOC) framing — accounting for acquisition, operation, maintenance, and end-of-life disposal — makes the service-as-subscription model legible in a way that pure product pricing doesn't. I argued then, and still believe, that as the economy shifted toward service-dominant logic, TOC literacy would become a competitive necessity across industries. The companies that could model their customers' true ownership costs and build service contracts around them would systematically outcompete those that couldn't.

### The Research Question I Couldn't Answer on the Job

By the time I left Servigistics to pursue a PhD, I had a list of questions I couldn't answer from inside a product organization. Why do firms systematically underinvest in service parts planning even when the ROI is demonstrable? How does the structure of warranty contracts shape the incentives of service providers — and through them, the operational decisions that determine whether customers get good service? What does "transparency" mean in a supply chain where every participant has information the others don't?

Those questions led me to Georgia Tech, to the work of Ravi Subramanian and Atalay Atasu, and eventually to the research agenda I'm still pursuing. The industry years didn't just inform my research — they gave it its urgency.
...
In the classroom, it means I can tell students not just what the theory says, but where the theory breaks — and why that matters. The student who goes on to manage a service parts network will not face a Poisson demand process; they'll face a messy mixture of intermittent demand, legacy data, and organizational politics. Knowing that the model is an approximation — and a useful one — is a different kind of knowledge than knowing the model.

The decade-plus I spent in industry is not a detour from the academic path. It is the reason the academic questions I ask are the ones I ask.
